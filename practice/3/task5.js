function runMarsohod(commands) {
    ...
    const sortCommands = commands.sort((first, second) => {
       return first.ORDER - second.ORDER;
    });

    const res = sortCommands.reduce((prev, curr) => {

        let [command, data] = curr.COMMAND.split(' ');
        ...
        if (command === 'GO') {
            ...
            if (prev.turn > 3) prev.turn = 0;
            if (prev.turn < 0) prev.turn = 3;
            return prev;
        } else {
            return prev;
        }
        ...
        coord: [0, 0],
        turn: 0        // 0 - вгору, 1 - вправо, 2 - вниз, 3 - вліво
    });
