function getSafePawns(pawns) {
    ...
    pawns.forEach((pawn) => {
        ...
        let p = pawn.split('');
        let down = Number(p[1]) - 1;
        let left = String.fromCharCode((p[0].charCodeAt(0)) - 1);
        let right = String.fromCharCode((p[0].charCodeAt(0)) + 1);
        //console.log(left);
        if (pawns.has(`${left}${down}`) || pawns.has(`${right}${down}`))
            sum++;
    });
    return sum;
}
